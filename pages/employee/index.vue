   ...
   <div class="col-lg-12">
      <div class="card" style="min-height:400px;">
         <div class="card-header" style="background-color:rgba(0,0,0,.03)">
          <p class="float-left">Employee List</p>
          <router-link to="/employee/create" type="button" name="button" class="float-right btn btn-info"> Add New Employee</router-link>
         </div>
            ...
            <table class="table table-responsive-sm table-bordered table-striped table-hover table-sm">
               ...
               <tbody>
                  ...
                  <tr  v-for="employee in employees"  :key="employee.id">
                     ...
                     <td>
                       <span class="badge badge-danger">{{employee.role ? employee.role.name : ''}}</span>
                     </td>
                     <td>{{employee.email}}</td>
                     <td>{{employee.mobile}}</td>
                     <td>
                       <router-link :to="`/employee/${employee.id}/edit`" type="button" name="button" class="btn btn-info">Edit</router-link>
                       <button type="button" class="btn btn-danger" @click="TheDelete(employee.id)">Delete</button>
                     </td>
                  </tr>
               </tbody>
            </table>
         ...
         <div class="card-footer" style="background-color:rgba(0,0,0,.03)">
           ...
           <nav class="float-left ml-4 mt-1">
             <p>Showing {{fromq}} - {{toq}} from {{totalq}} Employee </p>
           </nav>
...
export default {
  ...
  mounted () {
      if(!this.permission('staf-management')){
          // this.$router.push('/')
      }else{
          this.getData()
      }
