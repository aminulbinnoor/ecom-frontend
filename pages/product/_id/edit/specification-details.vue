   ...
   <div class="col-lg-12">
      <div class="card">
         <div class="card-header" style="background-color:rgba(0,0,0,.03)">
          <p class="float-left">Products Specification Details</p>
         </div>
         <div class="card-body p-0">
             <div class="form-row">
               <div class="col-md-12 p-1" style="">
                   <div class="form-group col-md-12 ">
                     <div class="input-group mt-2" v-for="(sp,index) in specification_details"  :key="sp.id">
                       ...
                       <div class="input-group-append">
                           <div class="btn-group" role="group" aria-label="Basic example">
                               ...
                               <button type="button" class="btn btn-info btn-sm" :disabled="index+1 !== specification_details.length"   @click="addNew()">+</button>
                          </div>
                       </div>
                     </div>
                   </div>
...
export default {
  data () {
    return {
      breadcrumb: [
          ...
          {name: 'Product Specification Details', link: ''},
      ],
  ...
  methods: {
    submit() {
      http1.post('/product/set-specification-details',{
    ...
    addNew(){
      this.specification_details.push({  name: '',value: ''})
    },
    removeNew(i){
      this.specification_details.splice(i,1)
    }
  },

  mounted: function () {
    http1.get('/product/get-specification-details',{
      params:{
        id: this.$route.params.id
      }
    }).then((response)=>{
        if(response.data.data.length > 0){
            this.specification_details = response.data.data
        }else{
            this.specification_details.push({ name: '',value: ''})
        }
    })
