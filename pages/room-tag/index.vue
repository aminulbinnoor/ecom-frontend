   ...
   <div class="col-lg-12">
      <div class="card">
         <div class="card-header" style="background-color:rgba(0,0,0,.03)">
          <p class="float-left">Room Tag List</p>
          <router-link to="/room-tag/create" type="button" name="button" class="float-right btn btn-info"> Add New Room Tag </router-link>
         </div>
         <div class="card-body p-0">
            <table class="table table-responsive-sm table-bordered table-striped table-hover table-sm">
               ...
               <tbody>
                  <tr v-for="roomtag in roomtags"  :key="roomtag.id">
                     <td>{{roomtag.id}}</td>
                     <td>{{roomtag.name}}</td>
                     <td>{{roomtag.short_description}}</td>
                     <td>{{roomtag.slug}}</td>
                     <td>
                       <span v-if="roomtag.is_active == 1" class="badge badge-success">Active</span>
                       <span v-else class="badge badge-danger">Inactive</span>
                     </td>
                     <td>
                       <router-link :to="`/room-tag/${roomtag.id}/edit`" type="button" name="button" class="btn btn-info btn-sm">Edit</router-link>
                       <button type="button" class="btn btn-danger btn-sm" @click="TheDelete(roomtag.id)">Delete</button>
                     </td>
                  </tr>
               </tbody>
            </table>
         ...
         <div class="card-footer" style="background-color:rgba(0,0,0,.03)">
           ...
           <nav class="float-left ml-4 mt-1">
             <p>Showing {{fromq}} - {{toq}} from {{totalq}} Room Tag</p>
           </nav>
  ...
  methods: {
    getData() {
      http1.get('room-tag/get',{
        ...
        this.roomtags = response.data.data.data
